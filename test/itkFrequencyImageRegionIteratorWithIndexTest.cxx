  ...
  public:
        ...
        for (unsigned int dim = 0; dim < ImageType::ImageDimension; dim++)
          ...
          if (it.GetFrequencyIndex()[dim] != itk::Math::abs(reverseIt.GetFrequencyIndex()[dim]))
            ...
            std::cout << "Failed testing hermitian property at index:" <<
              it.GetIndex() << " freq: " << it.GetFrequencyIndex()  << "\n"
              << " reverseIt: " << reverseIt.GetIndex() << " freq: " << reverseIt.GetFrequencyIndex() << std::endl;
            return false;
            }
          }
        --reverseIt;
    ...
    bool TestNegativeRegion(typename ImageType::RegionType & region)
    {
      ...
      while( !it.IsAtEnd() )
        ...
        if (it.GetIndex() == first_negative_index)
          ...
          if (m_LargestFrequency != it.GetFrequencyIndex() &&
             -m_LargestFrequency != it.GetFrequencyIndex())
            {
    ...
    bool TestLargestRegion()
    {
      ...
      if( it.GetIndex() == m_Image->GetLargestPossibleRegion().GetIndex() &&
          it.GetFrequencyIndex() != zero_freq_index )
        {
      ...
      while( !it.IsAtEnd() )
        ...
        if (index == it.GetHalfIndex() &&
            freq_index != m_LargestFrequency)
          {
          std::cout << "    Failed! Largest freq index is wrong. " << index <<
