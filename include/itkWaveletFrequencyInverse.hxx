...
::GenerateData()
{
  ...
  for (int level = this->m_Levels - 1; level > -1; --level)
    ...
    for(unsigned int band = 0; band < this->m_HighPassSubBands; ++band)
      ...
      // This is to normalize the multi-band approach. TODO is this generic? or depend on wavelet? Related with sub-band dilations. 2^(1/k) instead of Dyadic dilations. In any case this should be moved to waveletfilterbankgenerator
